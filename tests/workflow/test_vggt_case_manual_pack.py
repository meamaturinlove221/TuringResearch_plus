...
REPORT = ROOT / "docs" / "vggt-case-repo-manual-pack-report.md"
...
def test_vggt_case_manual_pack_has_required_files() -> None:
    required = {
        "README.md",
...
def test_vggt_case_manual_pack_keeps_manual_boundaries() -> None:
...
def test_vggt_case_manual_pack_has_safe_push_reference_only() -> None:
...
def test_vggt_case_manual_pack_is_public_safe() -> None:
    combined = "\n".join([_combined_text(PACK), _text(REPORT)])
    old_name = "Tuling" + "Research"
    private_drive = "D:" + "/vggt"
    smplx_payload = "SMPL" + "-X model files are included: true"
