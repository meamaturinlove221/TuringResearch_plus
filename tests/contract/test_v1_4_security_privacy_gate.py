...
TEXT_SUFFIXES = {".md", ".yaml", ".yml", ".json", ".toml", ".py", ".html", ".txt", ".sh"}
...
V1_4_PUBLIC_SURFACES = [
    ROOT / "README.md",
    ROOT / "docs" / "v1.4.0-full-production-replay-report.md",
    ROOT / "docs" / "original-repo-parity-dashboard-v2.md",
    ROOT / "docs" / "original-repo-production-parity-summary.md",
    ROOT / "docs" / "aris-still-deferred-v1.4.md",
    ROOT / "docs" / "v1.4.0-docs-polish-report.md",
    ROOT / "docs" / "session-production-parity-gate-report.md",
    ROOT / "docs" / "scholar-production-parity-gate-report.md",
    ROOT / "docs" / "web-production-parity-gate-report.md",
    ROOT / "docs" / "yogsoth-production-parity-gate-report.md",
    ROOT / "docs" / "v1.4.0-session-parity-summary.md",
    ROOT / "docs" / "v1.4.0-scholar-web-production-summary.md",
    ROOT / "docs" / "v1.4.0-yogsoth-production-summary.md",
    ROOT / "docs-site" / "pages" / "original-repo-parity-v2.md",
    ROOT / "examples" / "public_demo" / "parity_dashboard_v2.json",
    ROOT / "examples" / "session_runtime",
    ROOT / "examples" / "scholar_demo",
    ROOT / "examples" / "web_demo",
    ROOT / "examples" / "apify_workflows",
    ROOT / "examples" / "campaigns",
    ROOT / "examples" / "research_catalog",
    ROOT / "examples" / "vault_wiki_demo",
    ROOT / "examples" / "ontology_demo",
    ROOT / "examples" / "stress_convergence_demo",
    ROOT / "examples" / "experiment_execution",
]
...
V1_4_CODE_SURFACES = [
    ...
    ROOT / "contracts" / "session_cli_surface.yaml",
    ROOT / "contracts" / "session_script_export.yaml",
    ROOT / "contracts" / "cross_platform_archive_hardening.yaml",
    ROOT / "contracts" / "optional_remote_dry_run_plan.yaml",
    ROOT / "contracts" / "return_import_human_confirmation.yaml",
    ROOT / "contracts" / "heavy_pdf_backend_slot.yaml",
...
def test_v1_4_public_surfaces_have_no_privacy_release_blockers() -> None:
    report = scan_privacy_paths(V1_4_PUBLIC_SURFACES)
...
def test_v1_4_no_new_env_or_local_project_links_in_release_surface() -> None:
    offenders: list[str] = []
    for path in V1_4_PUBLIC_SURFACES + V1_4_CODE_SURFACES:
        candidates = (
            [path]
            if path.is_file()
...
def test_v1_4_no_token_or_api_key_values_in_production_surface() -> None:
    ...
    for path in _text_files_under(V1_4_PUBLIC_SURFACES + V1_4_CODE_SURFACES):
...
def test_v1_4_no_private_path_or_old_name_in_public_text() -> None:
    ...
    for path in _text_files_under(V1_4_PUBLIC_SURFACES):
...
def test_v1_4_no_raw_data_or_restricted_model_payloads_in_production_surface() -> None:
    offenders: list[str] = []
    for path in V1_4_PUBLIC_SURFACES + V1_4_CODE_SURFACES:
        candidates = (
            [path]
...
def test_v1_4_no_unsafe_remote_execution_or_default_live_ssh_sftp() -> None:
    combined = "\n".join(
...
def test_v1_4_no_paywall_bypass_or_default_live_network() -> None:
    combined = "\n".join(
        path.read_text(encoding="utf-8", errors="replace")
        for path in _text_files_under(V1_4_PUBLIC_SURFACES + V1_4_CODE_SURFACES)
    ).lower()
...
def test_v1_4_security_privacy_reports_exist_and_are_review_only() -> None:
    required = [
        ROOT / "docs" / "v1.4.0-security-audit.md",
        ROOT / "docs" / "v1.4.0-privacy-audit.md",
        ROOT / "docs" / "v1.4.0-secret-scan-report.md",
    ]
