...
def test_tool_registry_surface_is_testable() -> None:
    registry = importlib.import_module("turing_research.tool_registry")

    tools = registry.list_mcp_tools()
    tool_names = {tool.name for tool in tools}

    assert registry.MCP_SERVER_NAME == "turingresearch-plus"
...
def test_health_check_can_dry_run_through_mcp_entrypoint() -> None:
    mcp_server = importlib.import_module("turing_research.mcp_server")
