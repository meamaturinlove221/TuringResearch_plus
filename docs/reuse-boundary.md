# TuringResearch Plus Reuse Boundary
...
- Importing external project names into TuringResearch Plus package names, server names, tool names, skill names, or document titles.
...
Even when a public repository appears permissively licensed, TuringResearch Plus treats Round 4 as design-only. Any future code reuse requires file-level license verification and an explicit record in the relevant lane ledger before copying or adapting code.

## Naming Rule

All TuringResearch Plus implementation names must use:
...
All important TuringResearch Plus outputs become `ResearchArtifact`. Any conclusion must include `EvidenceRef`. Race Mode outputs must pass Source Hygiene Gate before becoming implementation work.
