# TuringResearch Plus MCP Local Smoke Test

This document records the local STDIO smoke path for the `turingresearch-plus` MCP server.

## Server

- MCP server name: `turingresearch-plus`
- Python module: `turing_research.mcp_server`
...
Other TuringResearch Plus tools may remain planned, contract-only, stubbed, or dry-run for `v0.1.0`; each public tool is marked with `implementation_status` in `docs/mcp-tools.md`.

## STDIO Safety Rules

- Importing `turing_research.mcp_server` must not start a network service.
...
```powershell
python -m turing_research.mcp_server
```
...
```toml
[mcp_servers.turingresearch-plus]
command = "python"
args = ["-m", "turing_research.mcp_server"]

[mcp_servers.turingresearch-plus.env]
TURINGRESEARCH_CACHE_DIR = ".turingresearch/cache"
TURINGRESEARCH_SESSION_REGISTRY_PATH = ".turingresearch/session_registry.json"
TURINGRESEARCH_FAKE_SERVICE = "1"
TURINGRESEARCH_API_KEY = "replace-with-local-secret-outside-repo"
```
