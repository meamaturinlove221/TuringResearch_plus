...
This scan refreshes TuringResearch Plus metadata about the local VGGT workspace so a human can decide whether `split_ready/turingresearch-vggt-case` needs to be updated.

## Scope

- observed: scan ran on a VGGT desktop with local co-location roots present.
- observed: candidate VGGT roots were read as local file metadata.
- observed: scan outputs were written only under TuringResearch Plus.
- observed: no VGGT code was executed.
- observed: no VGGT experiment was run.
- observed: no local VGGT workspace file was modified or deleted.
...
| Area | Status | Finding |
| --- | --- | --- |
| Co-location | local-observed | Local VGGT root candidates existed at scan time; exact machine paths are redacted in this cloud baseline. |
| V600 quality rebuild | local-observed | Recent quality-rebuild report metadata exists, including ready-not-promoted status wording. |
...
| Split-ready case marker | missing | No explicit `split_ready` or `turingresearch-vggt-case` file-name marker was found. |
...
- planned: keep raw data, SMPL-X model files, huge npz, ply, zip, and VGGT experiment archives outside TuringResearch Plus.
