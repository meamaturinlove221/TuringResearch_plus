...
VGGT dogfooding has strong review scaffolds but still needs a replay path for real exported artifacts.

## VGGT motivating example

A real VGGT-side run bundle should replay through run ingest, artifact audit, dashboard, comparison, advisor bundle, and evidence proposed updates.
...
- contracts/run_ingest.yaml, contracts/remote_artifacts.yaml, contracts/advisor_export.yaml
...
Replay can be confused with executing VGGT or proving SparseConv3D success.

## Done criteria

Fixture replay workflow, real-artifact dry-run policy, proposed updates only, no private path reads by default.
